...
const KitChecklist = ({ kitId, titre, checklist, couleur, drogues = [] }: Props) => {
  ...
  const onPrint = async () => {
    ...
    let doc: string;
    if (kitId === "isr" || kitId === "sedation-procedurale") {
      const printDocs = await import("./kitPrintDocs");
      doc =
        kitId === "isr"
          ? printDocs.buildIsrPrintDoc(values)
          : printDocs.buildSedationProcedurePrintDoc(values);
    } else {
      doc =
