...
import type { PrepRecipe } from "./PrepBlock.parts";
import {
  EmptyRecipe,
...
import { createPrepLegacyRenderers } from "./preparation/PrepLegacyRenderers";
...
const PrepBlock = ({
  ...
  produitFinal,
  prepPopulation,
  includePreviewOverrides: _includePreviewOverrides = true,
}: PrepBlockProps) => {
  ...
  const { prep } = useResolvedDrugPrep(drug);
  if (!prep) return null;
  ...
  } = createPrepLegacyRenderers({
    prep,
    kg,
    validKg: Boolean(validKg),
    effectivePrepInput,
    dosePrepInput,
